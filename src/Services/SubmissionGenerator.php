...
namespace Stats4sd\GenerateOdkSubmissions\Services;
...
class SubmissionGenerator {
    ...
    public function __construct(
        ...
        // The xlsform
        protected TeamXlsform $teamXlsform,
    ...
    )
    {
        ...
        $this->xPathHandler = new XPathExpressionHandler(variables: $this->variables, choices: $this->choices, teamXlsform: $this->teamXlsform);
...
dump($repeatCount);
    ...
    private function getChoicesList(array $variable): Collection
    {
        ...
        if (preg_match('/search\(\'(.+)\'\)/', $variable['appearance'], $matches)) {
            ...
            $lookups = collect($this->teamXlsform->xlsform->csv_lookups);
            $lookup = $lookups->where('csv_name', $matches[1])->first();

            $choiceQuery = DB::table($lookup['mysql_name']);

            if ((integer)$lookup['per_team'] === 1) {
                $choiceQuery = $choiceQuery->where('team_id', $this->teamXlsform->team->id);
            }
